...
/**
 * Email / stored PDF — match 「下載 PDF」(/print/quote) via the same print HTML.
 * Do not silently swap the attachment to PDFKit: a mail recipient must receive
 * the same visual quotation layout the user downloaded from the print page.
 */
export async function generateQuotePdfMatchingDownload(
  quote: any,
  ...
  try {
    ...
    console.error("[QuotePDF] Print-format PDF failed; refusing layout-changing fallback:", err);
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
      message:
        "無法生成與「下載 PDF」相同格式的報價單附件。郵件未發送，請稍後再試或先用下載 PDF 檢查排版。",
      cause: err,
    });
  }
}
